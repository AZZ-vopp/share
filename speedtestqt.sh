...
speed() {
    ...
    speed_test '59917' 'Viettel - HCM'
    speed_test '9903' 'Viettel - HN'
    speed_test '13984' 'Viettel - Đà Nẵng'
    speed_test '11342' 'VNPT - HCM'
    speed_test '17757' 'VNPT - HN'
    speed_test '24539' 'FPT - HCM'
    speed_test '2552' 'FPT - HN'
    speed_test '55436' 'MobiFone - HCM'
    speed_test '55434' 'MobiFone - HN'
    ...
    speed_test '22121' 'Moscow (Russia)'
...
ipv4_info() {
    ...
    if [[ -n "${city}" && -n "${country}" ]]; then
        echo " Quốc Gia           : $(_blue "${city} / ${country}")"
    fi
    if [[ -n "${region}" ]]; then
        echo " Thành phố          : $(_yellow "${region}")"
    fi
    if [[ -z "${org}" ]]; then
        echo " Thành Phố          : $(_red "Chưa đăng ký ISP")"
    fi
}
...
print_intro() {
    echo "-------------------- Lệnh kiểm tra tốc độ máy chủ -------------------"
    echo " Version            : v1.0"
    echo " Sử dụng bench.sh để mod lại 1 số phù hợp với Việt Nam"
}
...
print_io_test() {
    ...
    if [ "${freespace}" -gt 1024 ]; then
        ...
        echo " $(_red "Không đủ dung lượng cho bài kiểm tra Tốc độ I/O!")"
        echo " $(_red "Nâng bộ nhớ thêm đi !")"
    fi
}
...
print_end_time() {
    ...
    if [ ${time} -gt 60 ]; then
        min=$((time / 60))
        sec=$((time % 60))
        echo " Hoàn thành sau              : ${min} phút ${sec} giây"
    else
        echo " Hoàn thành sau          : ${time} giây"
    fi
    date_time=$(date '+%Y-%m-%d %H:%M:%S %Z')
    echo " Thời gian hiện tại          : $date_time"
}

! _exists "wget" && _red "Lỗi: không tìm thấy lệnh wget.\n" && exit 1
! _exists "free" && _red "Lỗi: không tìm thấy lệnh dự phòng.\n" && exit 1
# check for curl/wget
...
if [[ -z "$ipv4_check" && -z "$ipv6_check" ]]; then
    _yellow "Cảnh báo: Không phát hiện được cả kết nối IPv4 và IPv6.\n"
fi
